...
export async function GET(request: NextRequest) {
  ...
  const [statsRow] = await db
    .select({ total: count(), avgDuration: avg(siteVisitors.sessionDuration) })
    .from(siteVisitors)
    .where(periodFilter);
  ...
  const deviceBreakdown = await db
    ...
    .from(siteVisitors).where(periodFilter).groupBy(siteVisitors.device);

  const referrerBreakdown = await db
    .select({ referrer: siteVisitors.referrer, count: count() })
    .from(siteVisitors).where(periodFilter).groupBy(siteVisitors.referrer).orderBy(desc(count()));

  const cityBreakdown = await db
    .select({ city: siteVisitors.city, count: count() })
    .from(siteVisitors).where(periodFilter).groupBy(siteVisitors.city).orderBy(desc(count()));
  ...
  const browserBreakdown = await db
    ...
    .from(siteVisitors).where(periodFilter).groupBy(siteVisitors.browser).orderBy(desc(count()));
  ...
  const engineBreakdown = await db
    ...
    .groupBy(normalizedEngineExpr).orderBy(desc(count()));
  ...
  const topKeywords = await db
    ...
    .groupBy(normalizedKeywordExpr).orderBy(desc(count())).limit(20);
  ...
  const buttonBreakdown = await db
    ...
    .where(periodFilter)
    .groupBy(visitorClicks.buttonText).orderBy(desc(count()));
  ...
  const topPages = await db
    ...
    .where(periodFilter)
    .groupBy(visitorPageViews.path).orderBy(desc(count())).limit(10);
