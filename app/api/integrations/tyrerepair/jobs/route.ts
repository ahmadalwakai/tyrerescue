...
export async function POST(request: Request) {
  ...
  const notes = [originNote, body.externalRef ? `ext:${body.externalRef}` : null, body.notes]
    .filter(Boolean)
    .join(' ');
  ...
  try {
    const [created] = await db
      ...
      .values({
        ...
        tyreSizeDisplay: body.tyreSizeDisplay ? String(body.tyreSizeDisplay) : null,
        ...
        priceSnapshot: { source: 'tyrerepair.uk', externalRef: body.externalRef ?? null },
        subtotal,
        vatAmount,
        totalAmount: totalAmount!,
        paymentType,
        depositAmountPence:
          typeof body.depositAmountPence === 'number' ? body.depositAmountPence : null,
  ...
  try {
    ...
    if (!first) {
      ...
      await notifyDriverNewJob(driverId, refNumber, addressLine, driverPayment, bookingId);
    }
  } catch (pushError) {
